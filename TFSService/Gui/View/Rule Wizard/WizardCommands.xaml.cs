...
namespace Gui.View.Rule_Wizard
{
    ...
    public partial class WizardCommands : UserControl
    {
        ...
        {
            get { return (Transitioner)GetValue(TransitionerProperty); }
        ...
        private static void OnSubscribe(DependencyObject d, DependencyPropertyChangedEventArgs e)
        {
            ...
            {
                if (e.OldValue is Transitioner old)
                {
                    old.SelectionChanged -= commands.ChangeButtonsView;
                }

                if (e.NewValue is Transitioner newer)
                {
                    newer.SelectionChanged += commands.ChangeButtonsView;
                }
            }
        }
        ...
        private static void OnSubscribeForward(DependencyObject d, DependencyPropertyChangedEventArgs e)
        {
            if (d is WizardCommands commands)
            {
                ...
                {
                    var toRemove = commands.CommandBindings.OfType<CommandBinding>().FirstOrDefault(x => x.Command == old);
                    if (toRemove != null)
                    {
                        commands.CommandBindings.Remove(toRemove);
                    }
                }
        ...
        private void ShowOnly(params Button[] buttons)
        {
            ...
            foreach (var button in list.Except(buttons))
            {
